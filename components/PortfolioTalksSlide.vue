<template>
  <div
    class="flex flex-col items-center justify-center h-full my-auto px-4 text-center relative z-10 py-4"
  >
    <!-- Top Conference & Community Logos (Identical to Title Slide) -->
    <div class="flex items-center gap-2.5 mb-4 flex-wrap justify-center">
      <div class="title-chip">
        <img
          src="/coscup_logo.png"
          alt="COSCUP Logo"
          class="h-3.5 object-contain"
        />
        <span>COSCUP 2026</span>
      </div>
      <div class="title-chip flex items-center gap-1.5">
        <img
          src="/ruby_taiwan_logo.svg"
          alt="Ruby Taiwan Logo"
          class="h-4.5 object-contain"
        />
        <img
          src="/ruby_taiwan_logotype.svg"
          alt="Ruby Taiwan"
          class="h-3.5 object-contain filter invert brightness-200"
        />
      </div>
    </div>

    <!-- Title -->
    <h1 class="text-4xl font-black tracking-tight text-white mb-2">
      {{ currentLang === 'zh' ? '謝謝大家！ 🙏' : 'Thank You! 🙏' }}
    </h1>

    <p class="text-slate-300 text-sm mb-6 max-w-md mx-auto">
      {{ currentLang === 'zh' ? '掃描 QR Code 取得簡報檔與專案起手式 Code！' : 'Scan for presentation slide, and starter code.' }}
    </p>

    <!-- Minimalist Centered QR Code Card -->
    <div
      class="bg-slate-900/90 border-2 border-red-500/40 rounded-3xl p-6 shadow-2xl backdrop-blur-md flex flex-col items-center justify-center max-w-sm w-full space-y-4"
    >
      <!-- Scannable QR Code -->
      <div
        class="w-44 h-44 bg-white p-3 rounded-2xl shadow-xl flex items-center justify-center border border-slate-700"
      >
        <svg
          class="w-full h-full text-slate-950"
          viewBox="0 0 33 33"
          shape-rendering="crispEdges"
        >
          <path fill="#ffffff" d="M0 0h33v33H0z" />
          <path
            stroke="#000000"
            d="M4 4.5h7m6 0h1m4 0h7M4 5.5h1m5 0h1m5 0h4m2 0h1m5 0h1M4 6.5h1m1 0h3m1 0h1m1 0h1m1 0h1m3 0h3m1 0h1m1 0h3m1 0h1M4 7.5h1m1 0h3m1 0h1m1 0h1m4 0h1m1 0h2m1 0h1m1 0h3m1 0h1M4 8.5h1m1 0h3m1 0h1m1 0h5m3 0h1m1 0h1m1 0h3m1 0h1M4 9.5h1m5 0h1m1 0h2m3 0h1m1 0h2m1 0h1m5 0h1M4 10.5h7m1 0h1m1 0h1m1 0h1m1 0h1m1 0h1m1 0h7M12 11.5h1m2 0h1m2 0h3M4 12.5h1m1 0h5m2 0h2m2 0h3m2 0h5M5 13.5h1m1 0h1m1 0h1m2 0h1m3 0h1m1 0h1m1 0h1m2 0h1m3 0h1M5 14.5h4m1 0h1m1 0h1m5 0h2m1 0h2m1 0h2m1 0h2M4 15.5h3m2 0h1m1 0h1m8 0h5m3 0h1M4 16.5h1m1 0h2m1 0h6m2 0h2m2 0h2m1 0h1m1 0h3M4 17.5h1m1 0h1m1 0h1m2 0h1m1 0h2m1 0h1m4 0h1m1 0h1m1 0h1m1 0h1M4 18.5h1m1 0h1m1 0h1m1 0h2m7 0h7m1 0h2M4 19.5h1m1 0h1m1 0h2m1 0h2m3 0h1m1 0h3m1 0h3m3 0h1M4 20.5h1m1 0h3m1 0h2m1 0h1m2 0h9m1 0h1M12 21.5h2m3 0h1m1 0h2m3 0h2M4 22.5h7m4 0h2m1 0h1m1 0h1m1 0h1m1 0h1m1 0h3M4 23.5h1m5 0h1m1 0h1m2 0h1m4 0h1m3 0h2m2 0h1M4 24.5h1m1 0h3m1 0h1m1 0h2m1 0h10m1 0h1M4 25.5h1m1 0h3m1 0h1m1 0h2m2 0h2m1 0h1m1 0h2m1 0h5M4 26.5h1m1 0h3m1 0h1m1 0h1m4 0h1m2 0h1m4 0h2m1 0h1M4 27.5h1m5 0h1m2 0h4m1 0h3m1 0h4m2 0h1M4 28.5h7m1 0h1m1 0h1m2 0h2m2 0h8"
          />
        </svg>
      </div>

      <!-- Scannable URL Link -->
      <a
        href="https://netale.et/#talks"
        target="_blank"
        class="text-base font-mono font-bold text-red-400 hover:text-red-300 transition-colors underline tracking-wide"
      >
        netale.et/#talks
      </a>
    </div>
  </div>
</template>

<script setup>
import { currentLang } from '../composables/useLang'
</script>
